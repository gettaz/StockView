...
export class ClassificationService {
  ...
  getAllCategories(term: string): Observable<any> {
    ...
    return this.http.get<any[]>(`http://localhost:5130/api/AssetClassifications/8c656bc8-c03c-4909-bb0c-a2c4669316b5/classifications?classificationType=category`).pipe(
  ...
  getAllBrokers(term: string): Observable<string[]> {
    ...
    return this.http.get<any[]>(`http://localhost:5130/api/AssetClassifications/8c656bc8-c03c-4909-bb0c-a2c4669316b5/classifications?classificationType=broker`).pipe(
      tap((response: any) => console.log('Broker search results:', response)),
      map((brokers: string[]) => {
        if (term.trim()) {
          // Filter names that include the term
